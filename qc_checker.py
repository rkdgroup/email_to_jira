"""
The QC checker for DSLF list-rental tickets. One file, all of it.
...
Two questions, asked by the same command against the same ticket:
  ...
  SELECT CHECK  Did the PULL deliver what the ticket asked for? Ticket vs the SELECT
    ...
    python qc_checker.py                     # every ticket in Needs QC, posts its verdict
...
THIS FILE'S NAME IS LOAD-BEARING
The live Jenkins job "DSLF-Email-Scanner" is a freestyle Execute-shell step whose script
lives in the Jenkins job config, NOT in this repo's Jenkinsfile, and it calls
`python qc_checker.py` with no arguments. Renaming or splitting this file breaks the
scheduled build — that is exactly what happened on 2026-08-31. Editing the Jenkinsfile
does nothing.

EXIT CODE: FINDINGS ARE NOT A BUILD FAILURE
The job runs under `sh -xe`, so a non-zero exit reds the build. A ticket failing QC is a
result, not a build error, so main() returns 0 even when tickets FAIL or come back
UNVERIFIED. It returns non-zero only when the scan itself could not run: a bad config
(config_guard exits), bad arguments, or an unexpected exception.

THREE VERDICTS, AND THE THIRD IS THE POINT
PASS and FAIL are the model's. UNVERIFIED is the code's, and it is returned by every
failure path: no API key, timeout, API error, refusal, unreadable or oversize PDF, a failed
Jira read, a prompt that could not be assembled. UNVERIFIED IS NOT A PASS — it means QC
did not run and the ticket still needs a human. The re-run guard deliberately does not
count an UNVERIFIED comment as "already checked", so those tickets come back next run.
...
There was one (run_qc_checks, 14 regex checks). Its verdict was
...
Its SELECT-report regexes survive further down — they read printed values and make NO
judgements. Do not grow a comparison back into them.
"""
...
# ---------------------------------------------------------------------------
# SELECT report parsing
#
# What survived the removal of the rule-based checker: regexes that read a SELECT report's
# own printed values. This is NOT quality control — the QC above sends the PDF to the model
# and lets it read the report directly. These exist for two callers that need a specific
# number rather than a judgement: qty_approval_scanner imports parse_select_pdf for its
# TOTAL RECORDS SELECTED fallback, and check_ticket uses find_select_attachment to pick
# which PDF to send. A value read here belongs in a prompt as evidence, never in an
# if-statement that decides PASS or FAIL.
# ---------------------------------------------------------------------------
...
# Kept so existing callers and test_qc_checker.py keep working.
...
def _last_qc_comment_time(ticket_key: str) -> str | None:
    ...
    run (no API key, an API error, an unreadable PDF), and treating its comment as "checked"
...
def main() -> int:
    ...
    # Posting is the DEFAULT, as it was for the rule-based checker the cron used to call:
    # `python qc_checker.py` with no arguments has to post, because that is exactly how
    ...
    kw = dict(post=not args.dry_run, fix=args.fix, dry_run=args.dry_run,
    ...
    # Exit 0 even when tickets fail. The Jenkins job runs this under `sh -xe`, so a
    # non-zero exit reds the build — and a ticket failing QC is a result, not a build
    # error. The rule-based checker this replaced returned None and so always exited 0.
    # Non-zero is reserved for "the scan could not run": config_guard exits on a bad
    # YAML, argparse exits on bad arguments, and _entry below catches everything else.
    return 0


def _entry() -> int:
    """Process entry point. Only a scan that could not run may fail the build."""
    try:
        return main()
    except SystemExit:
        raise                       # config_guard / argparse — a real build failure
    except Exception:
        traceback.print_exc()
        log.error("QC scan crashed — failing the build")
        return 1
